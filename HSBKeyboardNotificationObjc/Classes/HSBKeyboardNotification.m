...
- (void)willShow:(NSNotification *)notification {
	...
	[self keyboardSize:notification animations:^(CGSize size) {
		if (self->_delegateFlags.showFlag) [self->_delegate hsbKeyboardNotificationShow:size];
	}];
}
...
- (void)willHide:(NSNotification *)notification {
	...
	_isKeyboardHidden = YES;
	[self keyboardSize:notification animations:^(CGSize size) {
		if (self->_delegateFlags.hideFlag) [self->_delegate hsbKeyboardNotificationHide:size];
	}];
}
